...
/*constructor(props){
        super(props)
        this.state = {
            displayPDF: csb_pdfPath
        }

    }

    togglePDF = () => {
        if(this.state.displayPDF === csb_pdfPath){
            this.setState({displayPDF: bee_pdfPath});

        }else{
            this.setState({displayPDF: csb_pdfPath});
        }
    ...
    render(){
        ...
        return(
            <div>
                ...
                <body>
                    <h1>{this.state.displayPDF}</h1>
                    <button onClick={() => this.togglePDF(PdfReader)} > click me </button>
                    <iframe title="PDF-Content" src={this.state.displayPDF} width="900" height="800"/>
                </body>
            </div>
